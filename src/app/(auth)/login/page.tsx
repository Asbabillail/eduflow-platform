...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (error) {
      setError(ar ? "تعذر إرسال رابط تسجيل الدخول." : "Could not send sign-in link.")
    } else {
      setSent(true)
    }
    setLoading(false)
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center bg-slate-50 px-4 ${ar ? "rtl" : "ltr"}`} dir={ar ? "rtl" : "ltr"}>
      ...
      <div className="w-full max-w-sm">
          ...
          {!sent ? (
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-sm text-slate-600">
                {ar ? `أرسلنا رابط تسجيل الدخول إلى ${email}` : `We sent a sign-in link to ${email}`}
              </p>
        ...
        <p className="text-center text-xs text-slate-400 mt-6">{ar ? "للدعم الفني، تواصل مع مسؤول النظام" : "For access, contact your school administrator"}</p>
      </div>
    </div>
  )
}